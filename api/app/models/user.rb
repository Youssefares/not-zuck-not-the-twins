...
class User < ActiveRecord::Base
  ...
  def isFriendsWith(friend)
    @friend = self.friendships.where(is_relationship_established:true, friend_id: friend)
    if @friend.first.present?
      return true
    else
      return self.inverse_friendships.where(is_relationship_established:true,user_id: friend).first.present?
    end
  end
